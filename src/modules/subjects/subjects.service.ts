...
async function assertClassInSchool(schoolId: string, classId: string): Promise<void> {
  const klass = await prisma.class.findFirst({ where: { id: classId, schoolId, deletedAt: null }, select: { id: true } });
...
async function assertTeacherInSchool(schoolId: string, teacherId: string): Promise<void> {
  const staff = await prisma.staff.findFirst({ where: { id: teacherId, schoolId, deletedAt: null }, select: { id: true } });
  if (!staff) throw new BadRequestError('Teacher must be staff in your school');
}
...
export async function createSubject(auth: AuthContext, input: CreateSubjectInput, ctx: AuditContext): Promise<unknown> {
  ...
  await assertClassInSchool(schoolId, input.classId);
  if (input.teacherId) await assertTeacherInSchool(schoolId, input.teacherId);
  const subject = await prisma.subject.create({
    data: { classId: input.classId, name: input.name, code: input.code, teacherId: input.teacherId ?? null },
  });
...
export async function updateSubject(auth: AuthContext, id: string, input: UpdateSubjectInput, ctx: AuditContext): Promise<unknown> {
  const schoolId = requireSchool(auth);
  const before = await prisma.subject.findFirst({ where: { id, class: { schoolId } } });
