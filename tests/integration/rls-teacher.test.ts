...
describe('teacher row-level security', () => {
  it('a teacher only sees students in classes they teach', async () => {
    ...
    const { user: teacher, staff } = await createStaffUser({ email: 't@s.test', password: 'Str0ng!Pass99', tenantId: tenant.id, role: 'TEACHER' });

    // Class the teacher is class-teacher of, with an enrolled student.
    const myClass = await createClassRow({ tenantId: tenant.id, classTeacherId: staff.id });
    const myStudent = await createStudentRow({ tenantId: tenant.id, studentNumber: 'MINE' });
    await prisma.enrollment.create({ data: { tenantId: tenant.id, studentId: myStudent.id, classId: myClass.id, academicYear: '2026' } });

    // Another class the teacher does NOT teach.
    const otherClass = await createClassRow({ tenantId: tenant.id, name: 'Other', section: 'Z' });
    const otherStudent = await createStudentRow({ tenantId: tenant.id, studentNumber: 'OTHER' });
    await prisma.enrollment.create({ data: { tenantId: tenant.id, studentId: otherStudent.id, classId: otherClass.id, academicYear: '2026' } });
  ...
  it('a teacher assigned via a subject can mark attendance for that class', async () => {
    const tenant = await createTenant();
    const { user: teacher, staff } = await createStaffUser({ email: 't2@s.test', password: 'Str0ng!Pass99', tenantId: tenant.id, role: 'TEACHER' });
    const klass = await createClassRow({ tenantId: tenant.id });
    await prisma.subject.create({ data: { tenantId: tenant.id, classId: klass.id, name: 'Art', code: 'ART', teacherId: staff.id } });
  ...
  it('a student can read only their own attendance', async () => {
    const tenant = await createTenant();
    const studentRow = await createStudentRow({ tenantId: tenant.id });
    const studentUser = await createUser({ email: 'kid@s.test', password: 'Str0ng!Pass99', role: 'STUDENT', tenantId: tenant.id });
    await prisma.student.update({ where: { id: studentRow.id }, data: { userId: studentUser.id } });
    const klass = await createClassRow({ tenantId: tenant.id });
    await prisma.enrollment.create({ data: { tenantId: tenant.id, studentId: studentRow.id, classId: klass.id, academicYear: '2026' } });
    await prisma.attendance.create({ data: { tenantId: tenant.id, studentId: studentRow.id, classId: klass.id, date: new Date('2026-05-08T00:00:00.000Z'), status: 'PRESENT', recordedBy: studentUser.id } });

    const own = await agentFor(app).get(`/api/v1/attendance?studentId=${studentRow.id}`).set(authHeader(studentUser));
    expect(own.status).toBe(200);

    const other = await createStudentRow({ tenantId: tenant.id, studentNumber: 'NOPE' });
